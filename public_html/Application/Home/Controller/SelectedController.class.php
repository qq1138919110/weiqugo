...
class SelectedController extends HomeController {
      ...
      //echo 123;exit;
       ...
       if(!$cid){
        $where_bannre['page']="home";
         ...
         $banner =M('Basic_set')->field('home_banner,link')->where($where_bannre)->order('number ASC')->select();
         ...
         //转换万的单位start
        foreach($main_push as $key=>$value){
                if($value['good_value']>10000){
                        $value['good_value']=$value['good_value']/10000;
                        $main_push[$key]['good_value']=sprintf("%.2f",substr(sprintf("%.3f", $value['good_value']), 0, -1)).'万';
                }
        }
         //转换万的单位end
        ...
        //转换万的单位start
        foreach($goods_type[$key]['goods_list'] as $k=>$val){
                 if($val['good_value']>10000){
                        $val['good_value']=$val['good_value']/10000;
                        $goods_type[$key]['goods_list'][$k]['good_value']=sprintf("%.2f",substr(sprintf("%.3f", $val['good_value']), 0, -1)).'万';
                }
        }
        //转换万的单位end
        ...
        foreach ($navigation as $k => $val) {
          ...
          //转换万的单位start
        foreach($navigation[$k]['goods_list'] as $k1=>$val1){
                 if($val1['good_value']>10000){
                        $val1['good_value']=$val1['good_value']/10000;
                        $navigation[$k]['goods_list'][$k1]['good_value']=sprintf("%.2f",substr(sprintf("%.3f", $val1['good_value']), 0, -1)).'万';
                }
        }
        //转换万的单位end
     ...
     //dump('123');exit;
      ...
      if($goods['is_good_attr']==1){
       //开启属性处理这里
        ...
        if(strpos($where_attr_val,',')){
          $where_attr_key['tp_item_attr_val.symbol']=array('in',$where_attr_val);
        }else{
          $where_attr_key['tp_item_attr_val.symbol']=$where_attr_val;
        }
       ...
       //dump($arrt_val_data);//exit;
        ...
        foreach ($new_attr_data as $key => $value) {
          $attr_data_new_new[]=$value;
        }
        $new_attr_data=$attr_data_new_new;
        //dump($new_attr_data);
  ...
  public function type_detail(){
     ...
     if(!isset($data_stem['navigation_id'])){
       if(isset($_GET['navigation_id'])){
         if($_GET['navigation_id']!=""){
           $where['navigation_id']=i('get.navigation_id');
         }
  ...
  public function get_attr(){
    $data_stem=i('get.');
    $where['good_id']=$data_stem['goods_id'];
    $where['good_status']=1;
    $where['recycle']=0;
    ...
    dump($goods_info);
    $where_attr['status']=0;
    $where_attr['good_id']=$data_stem['goods_id'];
    if($goods_info['is_good_attr']==1){
      //开启属性执行这里
      $goods_attr_str="";
      $attr_info=m('item_sku')->where($where)->select();
      foreach ($attr_info as $key => $value) {
        if($goods_attr_str==""){
          $goods_attr_str=$value['attr_symbol_path'];
        }else{
          $goods_attr_str=$goods_attr_str.','.$value['attr_symbol_path'];
        }
      }

    }else{
      //没有开启属性执行这里
      $attr_info[]=m('item_sku')->where($where)->order("sku_id DESC")->find();
      $goods_attr_str="";
    }

    if($goods_attr_str==""){
      $goods_attr_info['price']=$attr_info[0]['money'];
      $goods_attr_info['sku_id']=$attr_info[0]['sku_id'];
      $goods_attr_info['number']=$attr_info[0]['number'];
      $goods_attr_info['good_cover']=$goods_info['good_cover'];
      dump($goods_attr_info);
    }else{
      $re_data=d('Goods')->get_attr_sku($data_stem['goods_id'],$goods_attr_str);
      dump($re_data);
    }
    //dump($attr_info);
    //$attr_info=m('item_sku')->where($where)->select();
